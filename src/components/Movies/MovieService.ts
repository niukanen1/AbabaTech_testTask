import { queries, Response } from './../../Queries/GetPostQueries';
import { QueryFetch } from "../../Queries/GetPostQueries";
...
export type UserMoviesManipulationType = 'addFavoriteMovie' | 'deleteFavoriteMovie' | 'addWatchLaterMovie' | 'deleteWatchLaterMovie';
...
export async function GetLatestCompactMovies(
	page: number = 1,
	moviesSortType: MovieSortType
): Promise<GetMoviesResponseType | null> {
...
export async function GetSearchResults(query: string): Promise<SearchResult | null> {
	const queryURL = `https://api.themoviedb.org/3/search/movie?api_key=${process.env.REACT_APP_TMDB_API}&query=${query
		.split(" ")
		.join("%20")}`;
	try {
		const response = await fetch(queryURL);
...
// add, delete favorite and watch later movies
export async function MovieManipulation(
	manipulation: UserMoviesManipulationType,
	movieID: number,
...
// checks wether movie is belonged to any of user related lists (favoriteMovies, watchLater)
export function CheckUsersMovies(movieID: number): {isFavorite: boolean, isWatchLater: boolean} { 
    const favoriteMovies = AppStore.userData.user?.favoriteMovies
    const watchLaterMovies = AppStore.userData.user?.watchLaterMovies

    return {isFavorite: favoriteMovies?.includes(movieID) ?? false, isWatchLater: watchLaterMovies?.includes(movieID) ?? false};
}
